...
BERT 인코더부터 KG 저장까지 3단계 Incremental Stacking 구조로 설계된 NLP 파이프라인입니다.
...
| **기반 논문** | BERT, ATLOP (Zhou 2021), DREEAM (Ma 2023), GAIN (Zeng 2020) |
| **구조** | 3단계 Incremental Stacking (Stage 1 → 2 → 3) |
...
Baseline              ATLOP + DREEAM         + GNN
(BERT +               (LogSumExp +           (GAIN-lite
 MeanPool +            Adaptive TH +          그래프 추론)
 BCE Loss)             Evidence Head)
```
...
| 레이어 담당 | 파일 | 역할 |
|------------|------|------|
| `김예슬` | `model.py`, `utils.py`, `evaluation.py`, `scripts/` | 파이프라인 구축, 통합 모델, 학습/평가 스크립트, 도식화 |
...
│   ├── graph_encoder.py        # [Layer 3] GNN Graph Encoder (Stage 3)
...
│  - Stage 2/3: ATLOP Grouped Bilinear + Adaptive Threshold │
...
Stage 3:
  BERT → LogSumExp → GCN/GAT → rs 추출 → ATLOP Bilinear → 97 classes
                       ↑                                    ↑ Adaptive TH
                  Entity Graph                           + Evidence Head
            (Co-ref + Co-occur + Cross-sent)
```

### 설정 파일 핵심 비교

| 설정 키 | Stage 1 | Stage 2 | Stage 3 |
|---------|---------|---------|---------|
| `entity_repr.pooling` | `mean` | `logsumexp` | `logsumexp` |
| `relation_head.classifier_type` | `bilinear` | `atlop` | `atlop` |
| `relation_head.threshold_type` | `fixed` | `adaptive` | `adaptive` |
| `relation_head.use_evidence_head` | `false` | `true` | `true` |
| `graph_encoder.enabled` | `false` | `false` | `true` |
| `training.loss_type` | `bce` | `atlop` | `atlop` |
| `training.load_checkpoint` | - | - | Stage 2 ckpt |
...
| Kipf & Welling (2017) GCN | Graph Convolutional Network | Stage 3 |
| Veličković et al. (2018) GAT | Graph Attention Network | Stage 3 |
